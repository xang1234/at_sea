...
def get_port(chunk, speed_limit=SPEED_LIMIT, time_limit=TIME_LIMIT):
    """
...
def clear_duplicates(ports,port_threshold=PORT_THRESHOLD):
    """
    removes duplicate ports which are within PORT_THRESHOLD km of each other

    :param ports: dataframe of port coordinates (output of get_port function)
    :param port_threshold: distance in km to remove ports
    :return: dataframe of port coordinates
    """

    coords = ports[['latitude', 'longitude', 'lrimoshipno']].values
    for i, c1 in enumerate(coords):
        lat1, lon1 = c1[0], c1[1]
        for j, c2 in enumerate(coords):
            if i < j and coords[j, 2] > 0:
                lat2, lon2 = c2[0], c2[1]
                distance = haversine2(lat1, lon1, lat2, lon2)
                if distance < PORT_THRESHOLD:
                    coords[j, 2] = -1

    return ports[coords[:, 2] > 0]

def port_groupby_csv(session, params, s3_filename, key, agg,transport_params, chunk_size=1e6):
    """
    ...
    for chunk in tqdm(chunks):
        ...
        result = agg(chunk)
        res = clear_duplicates(pd.concat([res, result]))
...
def main():
    ...
    for i, f in enumerate(files):
        ...
        port = port_groupby_csv(session, params, f, 'lrimoshipno', get_port, transport_params,1e6)
        print(f)
